...
export default function Home() {
  ...
  const markAsDone = async (index: number) => {
    ...
    await axios.put("http://localhost:5000/api/tasks/mark-done", {
      data: {
        taskId: todo.taskId,
      },
    });
  ...
  const reactivateTask = async (index: number) => {
    ...
    await axios.put("http://localhost:5000/api/tasks/reactivate", {
      data: {
        taskId: todo.taskId,
      },
    });
  ...
  const handleEdit = async (index : number) => {
    ...
    const newTask = newActiveTodos[index];
    console.log(index);

    if (!input) return alert("Veuillez entrer une tâche.");
    if (!selectedColor) return alert("Veuillez choisir une couleur.");

    const response = await axios.put("http://localhost:5000/api/tasks/update", {
      data: {
    ...
    newActiveTodos[index] = { text: input, color: selectedColor, taskId: newActiveTodos[index].taskId };
                    ...
                    <DialogHeader>
                      ...
                      <div className="flex flex-col gap-4 mt-4">
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <Button onClick={() => handleEdit(taskindex)}>Edit</Button>
                        </div>
                      </div>
                    </DialogHeader>
                  </DialogContent>
